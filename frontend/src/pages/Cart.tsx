...
const Cart: React.FC = () => {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      const isPoolClosed =
        (statusCode === 400 && poolClosedFromMessage) || poolClosedFromRefetch;
      ...
      <div className="bg-white rounded-xl md:rounded-3xl max-w-md w-full p-8 shadow-2xl animate-in zoom-in-95 duration-200">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600 leading-relaxed">
            Unfortunately, this food pool is no longer accepting orders. The
            ordering window has ended.
          </p>
        </div>
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 mb-6">
          <p className="text-sm text-orange-800 font-medium leading-relaxed">
            ...
            <br />
            Check for the next available pool or browse other restaurants on
            your campus.
          </p>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="bg-white rounded-3xl shadow-soft p-8 animate-pulse">
            ...
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="flex gap-6 mb-6 pb-6 border-b border-gray-50"
              >
      ...
      <>
        <div className="text-center py-20">
          ...
          <p className="text-gray-500 mb-8 text-lg">
            Add some delicious food from the pools!
          </p>
          <button
            onClick={() => navigate("/")}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-8">
      ...
      <div className="bg-white border border-gray-100 rounded-xl md:rounded-3xl p-4 md:p-6 mb-6 md:mb-8 flex flex-row gap-2 md:gap-8 items-center justify-between md:justify-start shadow-soft">
        <div className="flex items-center gap-2 md:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5 truncate">
              POOL CLOSES
            </p>
            <p className="font-bold text-gray-900 text-lg md:text-xl leading-none mb-1 truncate">
              {pool ? formatLocalTime(pool.collection_end) : "..."}
            </p>
        ...
        <div className="flex items-center gap-2 md:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="font-bold text-gray-900 text-lg md:text-xl leading-none mb-1 truncate">
              {pool ? formatLocalTime(pool.expected_delivery_time) : "..."}
            </p>
            <p className="text-[10px] md:text-xs text-gray-500 font-medium truncate">
              Arrives at
            </p>
      ...
      <div className="flex flex-col lg:flex-row gap-8 relative">
        ...
        <div className="flex-grow space-y-6 min-w-0">
          {Object.entries(itemsByRestaurant).map(([restaurantId, items]) => {
            ...
            return (
              <div
                key={restaurantId}
                className="bg-white rounded-xl md:rounded-3xl shadow-soft border border-gray-50 overflow-hidden"
              >
                <div className="bg-gray-50/50 px-4 py-3 md:px-8 md:py-5 border-b border-gray-50">
                ...
                <div className="p-4 md:p-8">
                  {items.map((item) => (
                    <div
                      key={item.id}
                      className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:gap-6 py-4 md:py-6 border-b border-gray-50 last:border-0 last:pb-0 first:pt-0"
                    >
                      <div className="flex sm:items-center gap-3 md:gap-6 flex-1 min-w-0">
                       {/* image section */}
                        <div className="w-32 min-h-[8rem] sm:w-24 sm:h-24 bg-gray-100 rounded-xl md:rounded-2xl overflow-hidden flex-shrink-0 border border-gray-100 relative">
                          {item.dish.image ? (
                            <img
                              src={item.dish.image}
                              alt={item.dish.name}
                        ...
                        <div className="flex flex-col gap-4 flex-1">
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-4 md:gap-6 w-full sm:w-auto pl-0 sm:pl-0">
                            <div className="flex items-center bg-white rounded-xl border border-gray-200 h-8 md:h-10 shadow-sm">
                              <button
                                onClick={() =>
                                  updateQuantity(
                                    item.id,
                                ...
                                className="w-8 md:w-10 h-full flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-l-xl transition-colors"
                              >
                                <Minus className="w-3 h-3 md:w-4 md:h-4" />
                              </button>
                              <span className="w-6 md:w-8 text-center font-bold text-gray-900 text-sm md:text-base">
                                {item.quantity}
                              </span>
                              <button
                                onClick={() =>
                                ...
                                className="w-8 md:w-10 h-full flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-r-xl transition-colors"
                              >
        ...
        <div className="lg:w-[24rem] flex-shrink-0">
          <div className="bg-white rounded-xl md:rounded-3xl shadow-soft border border-gray-50 p-4 md:p-8 sticky top-24">
            ...
            <div className="space-y-3 md:space-y-4 mb-6 md:mb-8">
              ...
              <div className="flex justify-between text-gray-500 font-medium text-sm md:text-base">
                <span>
                  Delivery Fee{" "}
                  <span className="text-primary text-[10px] md:text-xs font-bold bg-primary-light px-1.5 py-0.5 rounded ml-1">
                    POOL
                  </span>
                </span>
                <span className="text-gray-900">₹{deliveryFee / 100}</span>
              </div>
              <div className="h-px bg-gray-100 my-4 md:my-6"></div>
            ...
            <div className="bg-gray-50 rounded-2xl p-3 md:p-4 mb-6 md:mb-8 text-center text-xs md:text-sm text-gray-500 border border-gray-100 font-medium">
              {restaurantNames.length > 0
                ? `Ordering from: ${restaurantNames.join(", ")}`
                : `Ordering from ${
                    Object.keys(itemsByRestaurant).length
                  } restaurant${
                    Object.keys(itemsByRestaurant).length !== 1 ? "s" : ""
                  }`}
            </div>
            ...
            <button
              ...
              className="w-full py-3 md:py-4 bg-primary text-white rounded-2xl font-bold text-base md:text-lg hover:bg-primary-dark transition-all shadow-lg shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed mb-4"
            >
            ...
            <div className="mb-8">
              <div className="bg-accent-light border border-accent/20 rounded-2xl p-5 mb-6">
                ...
                <p className="text-sm text-yellow-700/90 leading-relaxed">
                  We currently accept <span className="font-bold">Cash</span> or{" "}
                  <span className="font-bold">UPI payment on delivery</span>{" "}
                  only.
                </p>
              </div>

              <p className="text-gray-600 text-sm mb-6 leading-relaxed">
                By clicking "Confirm Order", your order will be placed and you
                can pay when your food arrives at the delivery hotspot.
              </p>
              ...
              <div className="bg-gray-50 rounded-2xl p-4 text-sm text-gray-600 border border-gray-100">
                ...
                <p className="text-xs text-gray-400">
                  Please keep exact change or UPI ready.
                </p>
              </div>
            </div>
